...
def impute_drug_forward(df, drug_forward_params, logfile, features):
    ...
    df_drugs_counts = pd.DataFrame(
        columns=['drug_name', 'lab_name', 'pat_counter', 'adms_counter', 'imputed_value', 'total_null_obs'])
    ...
    for lab_name in temp_df_meds['Lab Name'].unique():
        ...
        if (drug_forward_type == 'single_drug'):
            ...
            df_temp_masked_imputed = drug_forward_imputation(df_drugs.copy(), lab_name, df_med, delta_time_before,
                                                             mimic_data_querier, delta_time_after, drug_forward_type)
        ...
        if (drug_forward_type == 'multi_drugs'):
            ...
            for med_t_name in df_med.med_label:
                print('Create function for ' + lab_name + '<>' + med_t_name)
                # gam_func_temp = create_gum_func(df_inhuman,lab_name,med_t_name,delta_time_before,delta_time_after,mimic_data_querier)
                gam_func_temp = create_gum_func_df(lab_name, med_t_name, delta_time_before, delta_time_after,
                                                   mimic_data_querier)
                df_drugs = drug_forward_imputation(df_drugs, lab_name, med_t_name, delta_time_before, gam_func_temp,
                                                   delta_time_after, drug_forward_type)
            ...
            df_temp_masked_imputed.loc[:, df_temp_masked_imputed.columns == lab_name] = df_temp_masked_imputed[
                [lab_name + '_' + med_t_name for med_t_name in df_med.med_label.unique()]].mean(axis=1)
        ...
        for med_name in df_med.med_label.unique():
            ...
            imputed_values = (
                        df_temp_masked_imputed[lab_name].isna().value_counts()[1] - df[lab_name].isna().value_counts()[
                    1])
            lab_col_len = (df[lab_name].isna().value_counts()[0])
            print('For ' + lab_name + ' using the med ' + med_name + ': ' + str(
                imputed_values) + ' NaN values were imputed using drug_forward')
...
def compare_models_Kfold_validation(df, models_vector, imputation_methods,
    ...
    assert sorted(numerical_cols + categorical_cols + index_cols) == sorted(
        df.columns), "Error! Additional column should be dropped."
    ...
    for test_index, train_index in tqdm(splits_zipped):
        ...
        for imputation_method in imputation_methods:
            ...
            if risk_scores_df is None:
                ...
                risk_scores_df.loc[len(risk_scores_df)] = x
        split += 1
...
def calc_exponent_func(x, y, lab_name, med_t_name):
    ...
    plt.title("Fitted Exponential Curve " + f"\n R² = {round(rSquared, 3)}" + (
        f" | Y = {round(m, 3)} * e^(-{round(t, 3)} * x) + {round(b, 3)}"))
    plt.legend()
...
def extract_ratio_and_after_time_new(df_drug_lab, before_windows, after_windows, delta_time_before, delta_time_after):
    ...
    # calculate ratio
    df_drug_lab['ratio'] = df_drug_lab[f'after_abs_{after_windows[0]}_sp'] / df_drug_lab[
        f'before_abs_{before_windows[0]}_sp']
...
# drug forward
def drug_forward_imputation(df, lab_name, df_med, delta_time_before, mimic_data_querier, delta_time_after,
                            drug_forward_type):
    ...
    gams_dict = {}
    for med_t_name in df_med.med_label:
        gams_dict[med_t_name] = create_gum_func_df(lab_name, med_t_name, delta_time_before, delta_time_after,
                                                   mimic_data_querier)
    ...
    for pat_id in tqdm(df[~df[med_t_name].isna()]['subject_id'].unique(), mininterval=30):
        ...
        imputed_vec = fill_drug_forward_values_last_optimized(df_pat, df_med, lab_name, delta_time_after, gams_dict,
...
def extract_med_per_lab(lab_name, df_meds, df_d_items, inputevents_mv):
    ...
    df_med = df_meds[(df_meds['Lab Name'] == lab_name)][['Lab Name', 'Med Name']].rename(
        columns={'Med Name': 'med_label'})

    # pull drug itemid from d_items
    med_item_id = df_d_items[(df_d_items.LABEL.isin(df_med.med_label)) & (df_d_items.DBSOURCE == 'metavision')][
        ['LABEL', 'ITEMID']]
    ...
    df_inputevents = df_inputevents.groupby(['ITEMID', 'SUBJECT_ID', 'STARTTIME_rounded'])[
...
def add_med_adminstrations_cols(df_inputevents, df_data, df_med):
    ...
    for temp_item_id in df_med.ITEMID:
        ...
        df_data = pd.merge(df_data, temp_df_inputevents, how='left', left_on=['subject_id', 'charttime'],
                           right_on=['SUBJECT_ID', 'STARTTIME_rounded'])
...
def preprocessed_data_per_id(df_lab_data, pat_id, lab_name, df_med, delta_time_after, delta_time_before):
    ...
    df_pat = df_lab_data[(df_lab_data.subject_id == pat_id)][
        ['charttime', 'subject_id', lab_name] + df_med.med_label.to_list()]
...
def fill_drug_forward_values_last_optimized(df_pat, df_med, lab_name, delta_time_after, gams_dict, min_feature,
                                            max_feature):
    int_delta_time_after = int(delta_time_after[0])
    patients = df_pat['subject_id'].unique()

    # Precompute GAM predictions for all medications and time points
    gam_predictions = {
        med_t_name: [gams_dict[med_t_name].predict(np.array([[t]]))[0]
                     for t in range(int_delta_time_after + 1)]
        for med_t_name in df_med.med_label
    }

    # Prepare columns for imputed values
    cols_list = [f'd_{med_t_name}_{i}' for med_t_name in df_med.med_label for i in range(int_delta_time_after + 1)]
    df_pat[cols_list] = np.nan
    df_pat[f'{lab_name}d_forward'] = df_pat[lab_name]  # Initialize with lab values

    # Vectorized processing for each patient
    for pat_id in patients:
        df_patient = df_pat[df_pat['subject_id'] == pat_id].copy()

        for med_t_name in df_med.med_label:
            drug_indices = df_patient[~df_patient[med_t_name].isna()].index

            for idx in drug_indices:
                # Calculate imputed values for all future time points
                if not np.isnan(df_patient.loc[idx, lab_name]):
                    base_value = df_patient.loc[idx, lab_name]
                else:
                    base_value = df_patient.loc[idx, f'new_{lab_name}']
                imputed_values = [max(min(base_value * pred, max_feature), min_feature)
                                  for pred in gam_predictions[med_t_name]]

                # Assign imputed values to columns
                for t in range(len(imputed_values)):
                    col_name = f'd_{med_t_name}_{t}'
                    target_idx = idx + t
                    if target_idx < len(df_patient):
                        df_patient.loc[target_idx, col_name] = imputed_values[t]

        # Compute the average imputed values across medications
        d_forward_cols = [f'd_{med_t_name}_{t}' for med_t_name in df_med.med_label for t in
                          range(int_delta_time_after + 1)]
        df_patient[f'{lab_name}d_forward'] = df_patient[d_forward_cols].mean(axis=1, skipna=True)

        # Replace missing lab values with imputed ones
        df_patient[lab_name] = df_patient[lab_name].combine_first(df_patient[f'{lab_name}d_forward'])

        # Update the main dataframe
        df_pat.loc[df_patient.index, :] = df_patient
    ...
    for ind_split in risk_scores_df['Split'].unique():
        for imp_method in risk_scores_df['imputation_method'].unique():
            for model in list(models_vector.keys()):
                y_prob = risk_scores_df[
                    (risk_scores_df['Split'] == ind_split) & (risk_scores_df['imputation_method'] == imp_method)][model]
                y_target = risk_scores_df[
                    (risk_scores_df['Split'] == ind_split) & (risk_scores_df['imputation_method'] == imp_method)][
                    'Target']
...
def comparison_table(df_performance, metric, ver):
    res = df_performance.groupby(['imputation_method', 'model'])[metric].median().reset_index().sort_values(
        by='model').rename(columns={metric: 'median_' + metric})
    res['median_' + metric] = round(res['median_' + metric], 4) * 100
    mean = round(df_performance.groupby(['imputation_method', 'model'])[metric].mean().reset_index().sort_values(
        by='model').rename(columns={metric: 'mean_' + metric})['mean_' + metric], 4) * 100
    sd = round(df_performance.groupby(['imputation_method', 'model'])[metric].std().reset_index().sort_values(
        by='model').rename(columns={metric: 'std'})['std'], 4) * 100

    res_summary = pd.concat([res, mean, sd], axis=1)

    # res_summary['new'] = str(res_summary['median_auc'])+u"\u00B1"+str(res_summary['std'])
    res_summary['median_std'] = res_summary['median_' + metric].apply("{:.02f}".format).astype(str) + u"\u00B1" + \
                                res_summary['std'].apply("{:.02f}".format).astype(str)
    res_summary['mean_std'] = res_summary['mean_' + metric].apply("{:.02f}".format).astype(str) + u"\u00B1" + \
                              res_summary['std'].apply("{:.02f}".format).astype(str)
