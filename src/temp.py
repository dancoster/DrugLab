...
class Dataset:
    ...
    def __init__(self, name, data_path):
        ...
        self.preprocessed_inputevents = self.load_data('preprocessed_inputevents')
...
# len(list(temp['SUBJECT_ID'].value_counts().keys()))
...
# inputevents_mv_1 = inputevents_mv1
# inputevents_mv_1['STARTTIME'] = pd.to_datetime(inputevents_mv_1['STARTTIME'],  format='%Y/%m/%d %H:%M:%S')
# inputevents_mv_1['ENDTIME'] = pd.to_datetime(inputevents_mv_1['ENDTIME'],  format='%Y/%m/%d %H:%M:%S')
...
# finalDF[['SUBJECT_ID', 'HADM_ID', 'LABEL_x_x', 'LABEL_y_y', 'AMOUNT_x', 'AMOUNTUOM_x', 'UNITNAME_x', 'VALUENUM_x', 'VALUEUOM_x', 'timeFromPrescription_x', 
#        'CHARTTIME_x', 'STARTTIME_x', 'ENDTIME_x', 'CHARTTIME_y', 'VALUENUM_y', 'VALUEUOM_y',  'timeFromPrescription_y']]
...
# res = comp_analysis(lab_measurements, top200_meds[:60], 200)

# res

# res.to_csv(os.path.join(RESULT, 'Round-3', 'Inputevents_Med-Labtest_Pairs.csv'))
# # res_analysis = pd.read_csv(os.path.join(PARENT, 'Prescription_Med-Labtest_Pairs_Top100Drugs.csv'))
...
# final_res_df.to_csv(os.path.join(RESULT, 'Round-3',  'Inputevents_Med-Labtest-Pairs_Regression-Trend_Analysis_200.csv'))
